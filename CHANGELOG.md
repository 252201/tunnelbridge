...
- 桌面端支持编辑已有隧道，并在删除前提供应用内确认和执行状态反馈。
- 公网地址支持可靠复制，复制提示会在 3 秒后自动关闭。
- 管理后台支持先撤销、再永久删除设备；永久删除会一并清理关联隧道和监听端口。

### 修复

- 修复桌面端和管理后台长公网地址与其他列内容重叠的问题。
- 修复 macOS WebView 剪贴板权限异常时复制按钮无响应的问题。
